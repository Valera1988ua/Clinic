 ...
 */
    ...
    (() => {
        "use strict";
        const utils_isMobile = {
            Android: () => navigator.userAgent.match(/Android/i),
            BlackBerry: () => navigator.userAgent.match(/BlackBerry/i),
            iOS: () => navigator.userAgent.match(/iPhone|iPad|iPod/i),
            Opera: () => navigator.userAgent.match(/Opera Mini/i),
            Windows: () => navigator.userAgent.match(/IEMobile/i),
            any: () => utils_isMobile.Android() || utils_isMobile.BlackBerry() || utils_isMobile.iOS() || utils_isMobile.Opera() || utils_isMobile.Windows()
        };
        const utils_slideUp = function(target) {
        ...
        const utils_slideDown = function(target) {
            let duration = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 500;
            let showmore = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 0;
            if (!target.classList.contains("_slide")) {
                target.classList.add("_slide");
        ...
        const _slideToggle = function(target) {
            let duration = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 500;
            target.hidden ? utils_slideDown(target, duration) : utils_slideUp(target, duration);
        };
        ...
        function detectTouchDevice() {
            if (window.matchMedia("(pointer: coarse)").matches || "ontouchstart" in window) document.documentElement.classList.add("touch"); else document.documentElement.classList.remove("touch");
        }
        function burgerMenu() {
            ...
            if (burger) document.addEventListener("click", (_ref => {
                ...
                if (bodyLockStatus && target.closest(".burger-menu")) {
                    ...
                    document.documentElement.classList.contains("menu-open") ? burger.setAttribute("aria-label", "Open menu") : burger.setAttribute("aria-label", "Close menu");
                }
            }));
        }
        function accordion() {
            const accordionsRegular = document.querySelectorAll("[data-accordions]");
            if (!accordionsRegular.length > 0) return;
            const accordionsArray = Array.from(accordionsRegular).filter((item => !item.dataset.accordions.split(",")[0]));
            if (accordionsArray.length > 0) initAccordion(accordionsArray);
            document.addEventListener("click", accordionAction);
            let mdQueriesArray = utils_dataMediaQueries(accordionsRegular, "accordions");
            ...
            function initAccordion(accordionArray) {
                ...
                accordionArray.forEach((accordionBlock => {
                    ...
                    if (matchMedia.matches || !matchMedia) {
                        accordionBlock.classList.add("_accordion-init");
                        initAccordionContent(accordionBlock);
                    } else {
                        accordionBlock.classList.remove("_accordion-init");
                        initAccordionContent(accordionBlock, false);
                    }
                }));
            }
            function initAccordionContent(accordionBlock) {
                let hideAccordionContent = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : true;
                ...
                if (accordionItems.length) accordionItems.forEach((accordionItem => {
                    ...
                    if (hideAccordionContent) {
                        ...
                        if (!accordionItem.hasAttribute("data-open")) {
                            ...
                            setAriaAttributes(accordionControl);
                        } else {
                            accordionControl.classList.add("_accordion-active");
                            accordionItem.open = true;
                            setAriaAttributes(accordionControl, true);
                        }
                    } else {
                        accordionControl.setAttribute("tabindex", "-1");
                        accordionControl.classList.remove("_accordion-active");
                        accordionItem.open = true;
            ...
            function accordionAction(e) {
                ...
                if (target.closest("summary") && target.closest("[data-accordions]")) {
                    e.preventDefault();
                    if (target.closest("[data-accordions]").classList.contains("_accordion-init")) {
                        const accordionControl = target.closest("summary");
                        const accordionBlock = accordionControl.closest("details");
                        const accordionsParent = accordionControl.closest("[data-accordions]");
                        const oneAccordion = accordionsParent.hasAttribute("data-one-accordion");
                        const scrollAccordion = accordionBlock.hasAttribute("data-accordion-scroll");
                        const accordionSpeed = getSpeedAccordion(accordionsParent);
                        if (!accordionsParent.querySelectorAll("._slide").length) {
                            if (oneAccordion && !accordionBlock.open) hideAccordionContent(accordionsParent);
                            !accordionBlock.open ? accordionBlock.open = true : setTimeout((() => accordionBlock.open = false), accordionSpeed);
                            accordionControl.classList.toggle("_accordion-active");
                            _slideToggle(accordionControl.nextElementSibling, accordionSpeed);
                            accordionControl.classList.contains("_accordion-active") ? setAriaAttributes(accordionControl, true) : setAriaAttributes(accordionControl);
                            if (scrollAccordion && accordionControl.classList.contains("_accordion-active")) setScrollAccordion(accordionBlock);
                        }
                    }
                }
                if (!target.closest("[data-accordions]")) {
                    ...
                    if (accordionsClose.length > 0) accordionsClose.forEach((accordionClose => {
                        const accordionsParent = accordionClose.closest("[data-accordions]");
                        const accordionCloseBlock = accordionClose.parentNode;
                        if (accordionsParent.classList.contains("_accordion-init")) {
                            const accordionSpeed = getSpeedAccordion(accordionsParent);
                            accordionClose.classList.remove("_accordion-active");
                            utils_slideUp(accordionClose.nextElementSibling, accordionSpeed);
                            setTimeout((() => accordionCloseBlock.open = false), accordionSpeed);
                            setAriaAttributes(accordionClose);
                        }
                    }));
                }
            }
            function hideAccordionContent(accordionsParent) {
                const accordionActiveBlock = accordionsParent.querySelector("details[open]");
                if (accordionActiveBlock && !accordionsParent.querySelectorAll("._slide").length) {
                    const accordionActiveControl = accordionActiveBlock.querySelector("summary");
                    const accordionSpeed = getSpeedAccordion(accordionsParent);
                    accordionActiveControl.classList.remove("_accordion-active");
                    utils_slideUp(accordionActiveControl.nextElementSibling, accordionSpeed);
                    setTimeout((() => accordionActiveBlock.open = false), accordionSpeed);
                    setAriaAttributes(accordionActiveControl);
                }
            }
            ...
            function getSpeedAccordion(accordionParent) {
                return accordionParent.dataset.accordionSpeed ? +accordionParent.dataset.accordionSpeed : 500;
            }
            function setAriaAttributes(accordionControl) {
        ...
        function beforeAfter() {
            ...
            const beforeAfterArrow = beforeAfter.querySelector(".before-after__arrow");
            const afterItem = beforeAfter.querySelector(".before-after__item--after");
            ...
            if (!utils_isMobile.any()) {
                beforeAfter.addEventListener("mouseover", (_ref2 => {
                    let {target} = _ref2;
                    if (!target.classList.contains(".before-after__arrow")) {
                        if (target.closest(".before-after__item--before")) {
                            beforeAfterArrow.classList.add("before-after__arrow--left");
                            beforeAfterArrow.classList.remove("before-after__arrow--right");
                        }
                        if (target.closest(".before-after__item--after")) {
            ...
            if (beforeAfterArrow) utils_isMobile.any() ? beforeAfterArrow.addEventListener("touchstart", beforeAfterDrag) : beforeAfterArrow.addEventListener("mousedown", beforeAfterDrag);
            ...
            function beforeAfterArrowMove(e) {
                ...
                const posLeft = type === "touchmove" ? touches[0].clientX - beforeAfterSizes.left : clientX - beforeAfterSizes.left;
        ...
        class DynamicAdapt {
            ...
            init() {
                this.оbjects = [];
                this.daClassname = "_dynamic_adapt";
        ...
        class ScrollWatcher {
            ...
            scrollWatcherConstructor(items) {
                if (items.length > 0) {
                    ...
                    uniqParams.forEach((uniqParam => {
                        let [rootParam, marginParam, thresholdParam] = uniqParam.split("|");
                        let paramsWatch = {
                            root: rootParam,
                            margin: marginParam,
            ...
            getScrollWatcherConfig(paramsWatch) {
                ...
                let configWatcher = {};
                if (document.querySelector(root)) configWatcher.root = document.querySelector(root); else if (root !== "null") this.scrollWatcherLogging(`Эмм... батьківського об'єкта ${root} немає на сторінці`);
            ...
            scrollWatcherIntersecting(isIntersecting, target) {
                if (isIntersecting) {
                    !target.classList.contains("_watcher-view") ? target.classList.add("_watcher-view") : null;
                    this.scrollWatcherLogging(`Я бачу ${target.classList}, додав клас _watcher-view`);
                } else {
                    target.classList.contains("_watcher-view") ? target.classList.remove("_watcher-view") : null;
                    this.scrollWatcherLogging(`Я не бачу ${target.classList}, прибрав клас _watcher-view`);
                }
            }
            ...
            scrollWatcherLogging(message) {
                this.config.logging ? utils_FLS(`[Спостерігач]: ${message}`) : null;
            }
        }
        modules_objectModules.watcher = new ScrollWatcher({});
        ...
        function digitsCounter() {
            ...
            function digitsCountersAnimate(digitsCounter) {
                ...
                const step = timestamp => {
                    ...
                    progress < 1 ? window.requestAnimationFrame(step) : digitsCounter.removeAttribute("data-go");
                };
                window.requestAnimationFrame(step);
            }
            function digitsCounterAction(e) {
                const {entry: {target}} = e.detail;
                if (target.querySelectorAll("[data-digits-counter]").length) digitsCountersInit(target.querySelectorAll("[data-digits-counter]"));
            }
        ...
        function headerScroll() {
            ...
            document.addEventListener("windowScroll", (() => {
                ...
                const headerContactsHeight = header.querySelector(".contacts-header").offsetHeight;
                const menuBody = header.querySelector(".menu__body");
                if (scrollTop >= startPoint) {
                    !header.classList.contains("_header-scroll") ? header.classList.add("_header-scroll") : null;
                    if (menuBody.querySelector(".contacts-header") === null) {
                        header.style.cssText = `transform:translateY(${-headerContactsHeight}px)`;
                        if (headerShow) if (scrollTop > scrollDirection) header.style.cssText = `transform:translateY(${-headerContactsHeight}px)`; else header.style.cssText = `transform:translateY(0px)`;
                    }
                } else {
                    header.classList.contains("_header-scroll") ? header.classList.remove("_header-scroll") : null;
                    if (menuBody.querySelector(".contacts-header") !== null) header.style.cssText = `transform:translateY(0px)`;
                }
        ...
        const initSliders = () => {
            if (document.querySelector(".slider-hero")) {
                const heroSlider = new Swiper(".slider-hero", {
                    ...
                    pagination: {
                        el: ".control-hero__body",
                        clickable: false,
                        renderBullet: function(index, className) {
                            return `\n                <div data-slide=${index} class=${className}>${index < 10 ? "0" + (index + 1) : index + 1}\n                </div><span></span>\n               `;
                        }
                    },
                    on: {}
                });
                document.addEventListener("click", (_ref => {
                    let {target} = _ref;
                    if (target.closest(".control-hero__body") && target.classList.contains("swiper-pagination-bullet")) {
                        const slideId = +target.dataset.slide;
                        heroSlider.slideTo(slideId, 800);
                    }
                }));
            }
        };
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.required === "email") {
                    ...
                    if (this.emailTest(formRequiredItem) && formRequiredItem.value !== "") {
                        this.addError(formRequiredItem, "Не корректный email");
                        error++;
                    } else this.removeError(formRequiredItem);
                }
                if (formRequiredItem.dataset.required === "name") {
                    if (formRequiredItem.value !== "") this.addError(formRequiredItem, "Не корректное имя");
                    error++;
                }
                if (formRequiredItem.dataset.required === "phone") if (formRequiredItem.value.length === 10) {
                    this.addError(formRequiredItem, "Не корректный телефон");
                    error++;
                }
                if (formRequiredItem.type === "checkbox") if (!formRequiredItem.checked) {
                    this.addError(formRequiredItem, "Поствьте галочку");
                    error++;
                }
                if (!formRequiredItem.value.trim()) {
                    this.addError(formRequiredItem, "Заполните поле");
                    error++;
                }
                return error;
            },
            addError(formRequiredItem) {
                let textError = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : "";
                formRequiredItem.classList.add("_form-error");
                formRequiredItem.parentElement.classList.add("_form-error");
                let inputError = formRequiredItem.parentElement.querySelector(".form__error");
                if (inputError) formRequiredItem.parentElement.removeChild(inputError);
                if (formRequiredItem.dataset.error) formRequiredItem.parentElement.insertAdjacentHTML("beforeend", `<div class="form__error">${formRequiredItem.dataset.error ? formRequiredItem.dataset.error : textError}</div>`);
            },
            ...
            formClean(form) {
                ...
                setTimeout((() => {
                    ...
                    if (modules_objectModules.select) {
                        const selects = form.querySelectorAll(".select");
                        if (selects.length) selects.forEach((selectItem => {
                            const select = selectItem.querySelector("select");
                            modules_objectModules.select.selectBuild(select);
                        }));
                    }
                }), 0);
            },
        ...
        function formSubmit() {
            ...
            if (forms.length > 0) for (const form of forms) {
            ...
            async function formSubmitAction(form, e) {
                ...
                } else if (form.querySelector(".form-error") && form.hasAttribute("data-goto-error")) {
            ...
            function formSent(form) {
                ...
                setTimeout((() => {
                    if (modules_objectModules.modal) {
                        const modal = form.dataset.modalMessage;
                        modal ? modules_objectModules.modal.open(modal) : null;
                    }
                }), 0);
                formValidate.formClean(form);
        ...
        function formFieldsInit() {
            ...
            document.body.addEventListener("focusin", (_ref => {
                let {target} = _ref;
                if (target.tagName === "INPUT" || target.tagName === "TEXTAREA") {
                    if (!target.hasAttribute("data-no-focus-classes")) {
                        target.classList.add("_form-focus");
                        target.parentElement.classList.add("_form-focus");
                    }
                    formValidate.removeError(target);
                    target.hasAttribute("data-validate") ? formValidate.removeError(target) : null;
                }
            }));
            document.body.addEventListener("focusout", (_ref2 => {
                let {target} = _ref2;
                if (target.tagName === "INPUT" || target.tagName === "TEXTAREA") {
                    if (!target.hasAttribute("data-no-focus-classes")) {
                    ...
                    target.hasAttribute("data-validate") ? formValidate.validateInput(target) : null;
                }
            }));
            if (options.viewPass) document.addEventListener("click", (_ref3 => {
                let {target} = _ref3;
                if (target.closest('[class*="__viewpass"]')) {
                    let inputType = target.classList.contains("_viewpass-active") ? "password" : "text";
                    target.parentElement.querySelector("input").setAttribute("type", inputType);
                    target.classList.toggle("_viewpass-active");
                }
            }));
...
***************************************************************************** */
        ...
        if (galleries.length) {
            ...
            modules_objectModules.gallery = galleyItems;
        }
        const doctors = document.querySelector(".doctors");
        let loadDoctorsFlag = false;
        if (doctors) document.addEventListener("watcherCallback", (e => {
            const {entry: {target}} = e.detail;
            if (target.classList.contains("_watcher-view")) {
                loadDoctors();
                loadDoctorsFlag = true;
            }
        }));
        async function loadDoctors() {
            if (loadDoctorsFlag) return;
            try {
                ...
                doctorsInit(data.data);
            } catch (error) {
                console.error("There was a problem with the fetch operation:", error);
            }
        }
        function doctorsInit(data) {
            const doctorsList = document.querySelector(".body-doctors__list");
            data.forEach((doctor => {
                const {id, isActive, name, info, media: {avatar}} = doctor;
                let doctorsListTemplate = ``;
                if (isActive) buildDoctor(doctor);
                doctorsListTemplate += `\n               <a href="#" data-id=${id} class="body-doctors__item item-doctors-list ${isActive ? "_active" : ""}">\n                  <div class="item-doctors-list__image">\n                     <img src="img/home/${avatar}" alt="Image">\n                  </div>\n               <div class="item-doctors-list__body">\n                  <div class="item-doctors-list__name">${name}</div>\n                  <div class="item-doctors-list__info">${info}</div>\n               </div>\n            </a>\n         `;
                doctorsList.insertAdjacentHTML("beforeend", doctorsListTemplate);
            }));
            document.addEventListener("click", doctorsListActions);
            function doctorsListActions(e) {
                const {target} = e;
                if (target.closest(".item-doctors-list")) {
                    e.preventDefault();
                    const doctorItem = target.closest(".item-doctors-list");
                    const id = +doctorItem.dataset.id;
                    const activeDoctorItem = document.querySelector(".item-doctors-list._active");
                    if (!doctorItem.classList.contains("_active")) {
                        activeDoctorItem.classList.remove("_active");
                        doctorItem.classList.add("_active");
                        const [activeDoctor] = data.filter((doctor => doctor.id === id));
        ...
        function buildDoctor(activeDoctor) {
            const {name, media: {main, gallery}, skills, statistics} = activeDoctor;
            const doctorsMainPhoto = document.querySelector(".body-doctors__photo");
            const doctorsSkills = document.querySelector(".info-body-doctors__skills");
            const doctorsStatisticts = document.querySelector(".statistics-doctors");
            const doctorsGallery = document.querySelector(".works-doctors");
            let doctorSkillsTemplate = ``;
            let doctorStatistictsTemplate = ``;
            let doctorGalleryTemplate = ``;
            doctorsMainPhoto.innerHTML = "";
            doctorsSkills.innerHTML = "";
            doctorsStatisticts.innerHTML = "";
            doctorsGallery.innerHTML = "";
            main ? doctorsMainPhoto.innerHTML = `<img src="img/home/${main}" alt="${name}">` : null;
            for (const skill in skills) doctorSkillsTemplate += `\n              <div class="skills-item ${skills[skill] < 50 ? "skills-item--dark" : ""}">\n              <div style="min-width: ${skills[skill]}%" class="skills-item__info">\n                 <div class="skills-item__label">${skill}</div>\n                 <div class="skills-item__value">${skills[skill]}%</div>\n              </div>\n              <div style="width: ${skills[skill]}%;" class="skills-item__line"></div>\n           </div>\n         `;
            doctorsSkills.insertAdjacentHTML("beforeend", doctorSkillsTemplate);
            for (const stat in statistics) doctorStatistictsTemplate += `\n            <div class="statistics-doctors__item">\n              <div data-digits-counter class="statistics-doctors__value">${statistics[stat]}</div>\n              <div class="statistics-doctors__label">${stat}</div>\n          </div>\n         `;
            doctorsStatisticts.insertAdjacentHTML("beforeend", doctorStatistictsTemplate);
            const {small: smallArray, big: bigArray} = gallery;
            if (smallArray.length > 0) smallArray.forEach(((image, index) => {
                let imagePath = "";
                let imageDescr = "";
                for (const item in image) {
                    imagePath = item;
                    imageDescr = image[item];
                }
                doctorGalleryTemplate += `\n               <a href="img/home/${bigArray[index]}" class="works-doctors__image">\n                  <img src="img/home/${imagePath}" alt="${imageDescr}">\n              </a>            \n         `;
            }));
            doctorsGallery.innerHTML = doctorGalleryTemplate;
            modules_objectModules.watcher.scrollWatcherUpdate();
            modules_objectModules.gallery[0].galleryClass.refresh();
        }
        const aboutGallery = document.querySelector("[data-scroll-gallery]");
        if (aboutGallery && !utils_isMobile.any()) {
            const aboutGallerysWrapper = aboutGallery.querySelectorAll(".about-gallery__wrapper");
            window.addEventListener("load", (() => {
                aboutGallerysWrapper.forEach(((galleryWrapper, index) => {
                    const scrollAmount = +galleryWrapper.dataset.scrollAmount || 1;
                    let direction = index % 2 !== 0 ? 1 : -1;
                    let isScrolling = true;
                    if (direction === 1) galleryWrapper.scrollLeft = galleryWrapper.scrollWidth;
                    const scrollElement = () => {
                        if (!isScrolling) return;
                        galleryWrapper.scrollLeft += scrollAmount * direction;
                        if (galleryWrapper.scrollLeft + galleryWrapper.clientWidth >= galleryWrapper.scrollWidth) direction = -1; else if (galleryWrapper.scrollLeft <= 0) direction = 1;
                        window.requestAnimationFrame(scrollElement);
                    };
                    window.requestAnimationFrame(scrollElement);
                    galleryWrapper.addEventListener("mouseover", (() => {
                        isScrolling = false;
                    }));
                    galleryWrapper.addEventListener("mouseout", (() => {
                        isScrolling = true;
                        window.requestAnimationFrame(scrollElement);
                    }));
                }));
